...
printf("%s:%d: \n", __func__, __LINE__);
	...
	i = 0;
	cval = STS1_SYSPLL_STABLE|STS1_XHCI_RST|STS1_SYS125_RST|STS1_REF_RST|STS1_U3_MAC_RST;
printf("%s:%d: STS1=%x cval=%x\n", __func__, __LINE__, IPPCRD4(plat, MTK_XHCI_IP_PW_STS1), cval);
	while (i++ < 100) {
		val = IPPCRD4(plat, MTK_XHCI_IP_PW_STS1);
		if ((val & cval) == val)
			break;
		mdelay(1);
	}

printf("%s:%d: STS1=%x\n", __func__, __LINE__, IPPCRD4(plat, MTK_XHCI_IP_PW_STS1));
...
static int xhci_usb_probe(struct udevice *dev)
{
	...
	struct clk sys_ck, ref_ck, mcu_ck, dma_ck;

#define	GET_CLK(c)						\
	ret = clk_get_by_name(dev, #c, &c);			\
	if (ret)						\
		return ret;					\
	ret = clk_enable(&c);					\
	if (ret)						\
	...
	IPPCRD4(plat, MTK_XHCI_IP_XHCI_CAP);
	IPPCWR4(plat, MTK_XHCI_IP_PW_CTR0, IPPCRD4(plat, MTK_XHCI_IP_PW_CTR0) |
	    CTRL0_IP_SW_RST);
	...
	mdelay(1);
	IPPCWR4(plat, MTK_XHCI_IP_PW_CTR2, IPPCRD4(plat, MTK_XHCI_IP_PW_CTR2) |
	    CTRL2_IP_DEV_PDN);
	...
	printf("%s:%d %08x\n", __func__, __LINE__, IPPCRD4(plat, MTK_XHCI_IP_XHCI_CAP));
